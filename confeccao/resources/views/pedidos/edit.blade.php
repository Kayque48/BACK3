...
<head>
    ...
    <title>Editar Pedido - Confecção</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-header-text p { color: rgba(199, 125, 255, 0.7); font-size: 1rem; }
        ...
        .btn-back:hover { background: rgba(157, 78, 221, 0.1); box-shadow: 0 4px 12px rgba(157, 78, 221, 0.2); }
        ...
        input::placeholder, textarea::placeholder { color: rgba(199, 125, 255, 0.5); }
        ...
        .switch { position: relative; display: inline-flex; width: 50px; height: 28px; }
        ...
        .toggle-label { color: var(--color-purple-light); font-weight: 600; font-size: 0.95rem; }
        ...
        .btn-primary:hover { box-shadow: 0 8px 24px rgba(157, 78, 221, 0.4); transform: translateY(-2px); }
        ...
        .btn-secondary:hover { background: rgba(157, 78, 221, 0.1); box-shadow: 0 4px 12px rgba(157, 78, 221, 0.2); }

        .alert { padding: var(--spacing-md); border-radius: 0.5rem; border-left: 4px solid; margin-bottom: var(--spacing-md); display: flex; gap: 1rem; align-items: flex-start; }
        .alert-error { background: rgba(239, 68, 68, 0.1); border-color: #ef4444; color: #fca5a5; }
        .alert-icon { font-size: 1.2rem; flex-shrink: 0; }
        .alert-content ul { list-style-position: inside; margin-top: 0.5rem; }
...
<div class="container-glass">
    ...
    <div class="page-header">
        <div class="page-header-text">
            <h1><i class="fas fa-pen-to-square"></i> Editar Pedido</h1>
            <p>Atualize os dados do pedido abaixo</p>
        </div>
    ...
    <div class="glass-card">
        <div class="card-header">
            <span><i class="fas fa-shirt"></i> Dados do Pedido</span>
            <span class="badge-id"># {{ $pedidos->id }}</span>
        </div>

        <form action="{{ route('pedidos.update', ['pedidos' => $pedidos->id]) }}" method="POST">
            @csrf
            @method('PUT')
           ...
           <div class="form-group">
                ...
                <select id="cliente_id" name="cliente_id" required>
                    ...
                    @foreach($clientes as $cliente)
                        <option value="{{ $cliente->id }}"
                            {{ old('cliente_id', $pedidos->cliente_id) == $cliente->id ? 'selected' : '' }}>
                            {{ $cliente->nome }}
                        </option>
                    @endforeach
                ...
                @error('cliente_id')
                    <small style="color: #ef4444; margin-top: 0.25rem; display: block;">
                        {{ $message }}
                    </small>
                @enderror
            </div>
            ...
            <!-- Produto do Pedido -->
           <div class="form-group">
                <label for="produto_id">Produto <span class="required">*</span></label>
                ...
                <select id="produto_id" name="produto_id" required>
                    ...
                    @foreach($produtos as $produto)
                        <option value="{{ $produto->id }}"
                            {{ old('produto_id', $pedidos->produto_id) == $produto->id ? 'selected' : '' }}>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input
                        ...
                        id="quantidade"
                        name="quantidade"
                        value="{{ old('quantidade', $pedidos->quantidade) }}"
                        placeholder="0"
                        min="0"
                        required
                    >
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        <option value="em_andamento" {{ old('status', $pedidos->status) == 'em_andamento' ? 'selected' : '' }}>Em Andamento</option>
                        <option value="concluido" {{ old('status', $pedidos->status) == 'concluido' ? 'selected' : '' }}>Concluído</option>
                    </select>
                    @error('status')
                        <small style="color: #ef4444; margin-top: 0.25rem; display: block;">{{ $message }}</small>
                    @enderror
    ...
    <div class="info-box">
        ...
        <div>
            ...
            <p style="margin-top: 0.5rem;">Os campos marcados com <span style="color: var(--color-pink-accent);">*</span> são obrigatórios para atualizar o produto.</p>
        </div>
    </div>
